...
4.实现数据增量更新
1.实现获取最近一个月数据，并在web上可以以按钮形式启动。
2.获取数据应当先生成数据获取方案。哪些能聚合获取，哪些要按天获取。哪些是已经存在的数据。可以跳过无需重复获取。
3.web界面上实现一个初始化数据按钮。会获取最新三天数据。当首次启动项目后，可以通过这个按钮，来首次获取数据，来测试各项功能的生成是否正常。当数据库有数据后，此按钮不可点击。
4.实现一个更新数据，从最近一次数据获取试点，更新数据到按钮按下时，前一分钟的数据。比如说按钮按下时，是11:45，获取截止时间为11点44分59秒。
4.实现一个可选项，同步更新数据，勾选后，只要项目运行状态，每分钟同步一次最新数据。
5.要预估数据的请求数量，不要并发大量数据情况，导致上游服务器压力巨大，应当平滑请求
6.数据同步应该可以在web查看数据进度，有多少数据。当前进度是多少。正在同步什么数据，预计本项什么时间完成。预计整体什么时间完成。
7.是否还需要其他形式的数据增量功能在web上集成？给出需要的理由和不需要的理由。
